...
export class SocialServiceManager extends BaseCardManager {
        ...
        if (!document.getElementById('socialServiceModal')) {
            this.modalManager.createModal('socialServiceModal', `
                <div class="modal-content" style="max-width: 580px; background: linear-gradient(135deg, #1a2a3a, #0d1b2a); border-radius: 28px; text-align: center; border: 2px solid #ffb347; padding: 24px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 10px;">
                        <div id="socialChoiceInvestment" style="cursor: pointer; transition: all 0.3s ease; border-radius: 16px; overflow: hidden; border: 2px solid #ff6f00; background: rgba(255,111,0,0.1);">
                            ...
                            <div style="padding: 12px; display: flex; justify-content: center; align-items: center; min-height: 120px; background: rgba(0,0,0,0.3);">
                                <img src="/cards/cover/investment.png" alt="项目投资卡" style="width: 100%; max-width: 140px; height: auto; border-radius: 8px; border: 1px solid #ff6f00;" onerror="this.style.display='none'; this.parentElement.innerHTML='<div style=\\'font-size: 48px;\\'>🏗️</div><div style=\\'font-size: 12px; color: #ffd966;\\'>项目投资</div>'">
                            </div>
                            <div style="padding: 10px; background: rgba(0,0,0,0.4); text-align: center; font-size: 12px; color: #aaa;">大型投资项目 · 高回报</div>
                        </div>
                        <div id="socialChoiceSocial" style="cursor: pointer; transition: all 0.3s ease; border-radius: 16px; overflow: hidden; border: 2px solid #2e7d32; background: rgba(46,125,50,0.1);">
                            ...
                            <div style="padding: 12px; display: flex; justify-content: center; align-items: center; min-height: 120px; background: rgba(0,0,0,0.3);">
                                <img src="/cards/cover/social.png" alt="服务社会卡" style="width: 100%; max-width: 140px; height: auto; border-radius: 8px; border: 1px solid #2e7d32;" onerror="this.style.display='none'; this.parentElement.innerHTML='<div style=\\'font-size: 48px;\\'>🤝</div><div style=\\'font-size: 12px; color: #81c784;\\'>服务社会</div>'">
                            </div>
                            <div style="padding: 10px; background: rgba(0,0,0,0.4); text-align: center; font-size: 12px; color: #aaa;">社会公益 · 造福人群</div>
                        </div>
                    </div>
                    ...
                    <div style="margin-top: 10px; font-size: 11px; color: #666; text-align: center;">💡 点击上方卡片选择类型</div>
                </div>
            `);
        }
    }
    ...
    showSocialServiceModal(message) {
        ...
        const handleChoice = (choice) => {
            ...
            if (choice === 'investment') this.ui.addLog('🏛️ 你选择抽取项目投资卡', 'event');
            else if (choice === 'social') this.ui.addLog('🏛️ 你选择抽取服务社会卡', 'event');
            else this.ui.addLog('❌ 已取消社会服务中心', 'warning');
        };
