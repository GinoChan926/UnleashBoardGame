...
function handleRoll(ws, data, roomId) {
    ...
    // 对于骗子卡，不发送 dice_result（因为卡片执行会有自己的消息）
    if (tile.type !== 'opportunity' && tile.type !== 'lier') {
        ws.send(JSON.stringify(result));
        broadcastToRoom(roomId, result, ws);
    ...
    // 骗子卡已经在 drawAndExecuteLierCard 中发送了完整消息，这里不再重复发送
